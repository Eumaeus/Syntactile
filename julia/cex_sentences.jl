...
# ╔═╡ 39c8252d-9802-40a4-b91d-233baba0954e
begin
	using BetaReader
	using ArgParse
end

# ╔═╡ ae3a6f06-d369-490a-82c6-51e11af18dad
md"""
## 0. Configs
"""

# ╔═╡ b255e998-8042-4f57-b1ee-a216e07dc37d
output_fn = "Hansen_Quinn_Sentences"

# ╔═╡ d481c457-d5a6-4517-9607-d0e0823c5e8b
output_label = "// Hansen & Quinn, "

# ╔═╡ fe552087-2704-49a3-adb5-716c5f376bfa
input_file = "/Users/cblackwell/cite/grok/Syntactile/src/texts/HQ_Sentences_tokenized.cex"

# ╔═╡ 0bdd783b-3f97-4d58-a3d9-b9bed21774cd
local_text_file = "texts/HQ_Sentences_tokenized.cex"

# ╔═╡ aae21ffc-b9d5-4887-b95c-14c8ca75c735
base_urn = "urn:cts:fuTeaching:blackwell.hq.2026:"

# ╔═╡ 94fe1b2d-aabd-41db-a420-48becd12855b
sentence_urn_regex = "($base_urn[0-9]+\\.[0-9a-z]+)\\." |> Regex

# ╔═╡ a68d3eac-448e-45f7-be93-a22b9a770321
output_filename_regex = "($base_urn([0-9]+)\\.)" |> Regex

# ╔═╡ 19f28cd0-50df-486e-ae11-953e06cde255
output_dir = "/Users/cblackwell/cite/grok/Syntactile/src/sentences/"

# ╔═╡ 55e9f51c-f587-48c6-9274-4c20826bbc44
md"""
## 1. Load a CEX file
"""

# ╔═╡ 6cc3d05a-b43a-4363-af26-cd274aae4cf6
md"Get a tokenized text."

# ╔═╡ ec45a922-c759-4604-8bc2-0f166dc60607
md"A function to load a CEX text, skipping the header and getting only the data lines."

# ╔═╡ d7a8a514-063e-4685-a0f0-b0ca9d06207e
...
# ╔═╡ 07df2618-05be-4f1c-a236-4041545e9be4
md"Get the data…"

# ╔═╡ 0f3a4fc8-617c-4d1d-a2d7-d59c88a272b9
cex_lines = load_cex(input_file)

# ╔═╡ dcea0d1b-2ec9-4720-8d0c-c8f7b4d9d1a6
md"Just the URNs…"

# ╔═╡ da5f95f8-9acd-44c9-8cb0-340ed2d80dd6
urn_list = map(tup -> tup[1], cex_lines)

# ╔═╡ a68bce64-cf4f-42b9-8d95-d1e02e0df501
md"Get a list of sentence URNs…"

# ╔═╡ cbb795da-e88b-452c-af54-80f1461225e0
sentence_urns = map(urn_list) do urn
	m = match(sentence_urn_regex, urn)
	m[1]
end |> unique

# ╔═╡ a9b86188-2b71-4e37-aa85-cfbe882ffd52
md"Get the tokens for each sentence-urn; turn them into a range-urn"

# ╔═╡ 24abfacd-cbef-4157-a4c2-2d08203ad45b
range_urns = map(sentence_urns) do sent
	this_sentence_tokens = filter(urn_list) do urn
		contains(urn, (sent * "." ))
	end
	first_cite = this_sentence_tokens[1]
	end_cite = replace(this_sentence_tokens[end], "urn:cts:fuTeaching:blackwell.hq.2026:" => "")
	first_cite * "-" * end_cite
end

# ╔═╡ b959db23-4116-4079-8c90-019f2182dfcd
md"""
## Output Files by Citation-Level
"""

# ╔═╡ 94e39ed8-4ba0-4b1a-a8cd-70db678cedb4
filegroups = map(sentence_urns) do urn
	# Get a filename for this group
	m = match(output_filename_regex, urn)	
	m[1]
end |> unique
...
manifest_format = "2.0"
project_hash = "d13d077c7aa4953c0b8864907849e3cf0bf6d381"
